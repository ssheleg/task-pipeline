...
| REQ-054 | Every negative self-test is classified from the AST by whether it READS a value a release can move — provenance tracked from `open(...).read()` through derived names, read-backs tracked per path, regex shape stripped, a grep over the validator's own output excluded as assertion 3 | `2026-08-31-b113-derived-anchors` | v1.81.0 | — | local | pass | never | B-113 · `test/anchors_test.py` — 18 whole-workflow fixtures, 8 watched firing; 4 of them are retractions of this detector's own first draft (a `{7,40}` quantifier read as a year, `\u2192` in a raw string read as 2192, a payload built from local strings, a validator-output grep). Census over the corpus at the branch point: **26 of 419 plants** pinned a moving value |
| REQ-055 | The census prints as a measurement beside the verdict — plants, needles read off disk, pinned, declared, dormant-capable, and how many yield no text needle — with no floor and no target | `2026-08-31-b113-derived-anchors` | v1.81.0 | — | local | pass | never | B-113 · `negative-test anchors: 422 plants · 891 needles read off disk · 1 pinned to a value a release can move · 1 declared undrivable · 4 can decline to run · 39 yield no text needle (their look is a JSON key, which raises rather than passing)  (disclosure — no floor, no target)`; property check `the anchor census prints its disclosure beside the verdict` requires both halves of the line |
