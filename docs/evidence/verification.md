...
| CS-2 | A reworded sentence fails loudly rather than passing quietly | deleting the `all 39 references` clause gives *no longer states references in a shape this check can read* — the shape that twice made a healthy guard report itself broken | yes |
...
| REQ-054 | Every negative self-test is classified from the AST by whether it READS a value a release can move — provenance followed from `open().read()`, a `with`-handle, `pathlib.Path`, `json.load`/`yaml.safe_load` and a compiled pattern, through derived names, slices, comprehensions and builtin wrappers; read-backs keyed per path by its own source text; regex shape stripped; a grep over the validator's own output excluded as assertion 3; and a name rebound to something off no disk LOSES its provenance, which is what a `for` target needed rather than a scope Python does not give it | `2026-08-31-b113-derived-anchors` | v1.81.1 | — | local | pass | never | B-113 · `PASS: anchors.py — 47 cases, 27 of them watched firing`; 18 of the 45 cases assert silence and 16 of those 18 are retractions of this detector's own drafts, the other two being positive controls. Census at the branch point: **26 of 419 plants** pinned a moving value |
| REQ-055 | The census prints as a measurement beside the verdict, and every part of the line is COMPUTED — including why a plant yields no needle | `2026-08-31-b113-derived-anchors` | v1.81.1 | — | local | pass | never | B-113 · `negative-test anchors: 423 plants · 832 needles read off disk · 1 pinned to a value a release can move · 1 declared undrivable · 4 can decline to run · 41 yield no text needle (13 shell only, no python body, 11 a python body that reads no file, 10 a whole-file byte comparison, 6 provenance stopped at a helper function, 1 a JSON or dict key, which raises when it goes stale)  (disclosure — no floor, no target)`. The five reasons are derived, not glossed: the first draft explained all of them as *a JSON key, which raises rather than passing* and named the cause of exactly one plant of the 39 it counted. Property check `the anchor census prints its disclosure beside the verdict` requires the line |
| REQ-056 | An undeclared anchor is refused by the cheap gate; a declaration that resolves to nothing the plant reads is refused; and so is a reason padded to length with filler | `2026-08-31-b113-derived-anchors` | v1.81.1 | — | local | pass | never | B-113 · watched red before shipping: plants `anc1` (a derived `\d+` re-pinned to `22`, and it asks the census itself whether a new refusal appeared rather than trusting the first candidate) and `anc3` (an `# anchor:` target rewritten to a value no needle carries). 25 of 26 pinned plants derived; one declared — `res15`'s `^13\.`, because the criterion number is the GUARD's contract and deriving it would prove another criterion's guard fired. A reason is judged on characters AND distinct words, because 60 dots satisfy a length floor — the R-005 reader passed one |
| REQ-057 | A dormant check is never counted as a passing one — named in the runner's own summary, subtracted from its claim, and required to declare the state it cannot construct | `2026-08-31-b113-derived-anchors` | v1.81.1 | — | local | pass | never | B-113 · `test/negatives.py` gained a `dormant` bucket for plants AND for property checks (the second found by the R-005 reader: a property check printing `SKIP:` still counted as one that printed what it asserts), and the arithmetic lives in two pure functions because **the dormant branch cannot be reached on a healthy machine** — all four skip-capable plants ran and fired when the reader drove them end to end. `test/runner_test.py` calls those functions directly: 15 cases, three of them the empty-set branch that used to print `PASS: K DORMANT` over nothing. Plant `anc2` watches the declaration half red |
| REQ-058 | The three v1.80.0 instances and the live-board plants beside them stay covered | `2026-08-31-b113-derived-anchors` | v1.81.1 | — | local | pass | never | B-113 · `bd5` was already derived and is untouched by this change; `bd6` (its template sibling) now derives the same way; `pf1` still constructs its own no-blind-run precondition and is untouched — an earlier draft of this row claimed it gained a dormancy declaration, which was false and the R-005 reader measured it. `gap1` and `gapmention` keep their constructed preconditions and gained `# dormant-when:` declarations, whose stated cause was corrected after measurement: the local runner rebuilds `.git` and CI fetches tags, so neither path reaches the skip. `bd10` (the `B-008` row) and `tbl`/`tbl2` (the `B-015` row) were pinned to LIVE board rows and are derived — all three would have stopped landing the day those rows closed |
| REQ-059 | The R-005 independent reader was dispatched on the widened check and every blocking finding was closed before merge, over four rounds | `2026-08-31-b113-derived-anchors` | v1.81.1 | — | local | pass | never | B-113 · **round 1, five blocking defects the author's own 18 fixtures did not find** — provenance lost through `enumerate()`, with two live plants (`wv1`, `wv2`) still pinned to `B-005` and invisible; `pathlib.Path(...).read_text()` and `re.compile` unread; `bd2` and `bd13` collapsed onto one row; three documents restating a count of 21 the detector put at 24. **Round 2, four more** — `json.dump(d, open(p, "w"))` unrecognised as a write (39 plants write that way, none flagged yet: latent, and said as latent); a loop or comprehension variable whose provenance never died; a no-needle disclosure that named the cause of 1 of the 39 plants it counted; a fixture count written 18/8 in one sentence and 35/20 in the next. **Round 3, four numbers and one false negative** — the closure line still said 18/8; the silence/retraction figure was 44−25 rather than counted; the `json.dump` claim had been upgraded from latent to live; and scoping a `for` target killed provenance Python itself keeps. **Round 4, three numbers carried across a commit that moved the counter.** **DEVIATION, stated:** `build.md` calls for a FRESH reader at rounds 4–5 and this was the same reader resumed — its three prior reports are in its context, which is the independence R-005 exists to protect. Its round-4 findings were confirmed by re-running the commands here rather than accepted. The census figure moved 27 → 21 → 24 → **26** and the fixture count 18/8 → 44/25 → **47/27**; both are registered claim classes now, so the fifth occurrence is a failing gate |
