...
| REQ-001 | The record behind the manifesto's opening story reads as one event — both observation moments stated (the inventory one minute after the monitor was armed, `ps` at the elapsed time its own receipt quotes), the interval named, and the wall-clock times it never carried named as absent | `2026-08-19-residue-record` | unreleased | `f0402c2` | pass | never | TP-02 · M-40 · `npm test` exit 0 (`graph.py — 129 cases`); five new plants watched refused via `test/negatives.py -k receipt|record|containers`, and 40 further count- and residue-adjacent plants re-run green. A sixth plant was found already broken and repaired: `cl1` reads `Guards: N → **M**` out of the newest CHANGELOG section, and the post-tag note introduced with B-080 bolded the whole phrase, so the check had been proving nothing since that commit |
