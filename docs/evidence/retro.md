...
that reads mentions as declarations. The tag was re-pointed at the repaired
tree before anything published from it; nothing had reached npm or the release
page from the failed round.
...
**The trailing gap right now is `v1.76.1`, `v1.77.0`, `v1.78.0`, `v1.78.1`, `v1.78.2`, `v1.78.3` and `v1.78.4` — seven releases.**
...
| 2026-08-22 | `doctrine-from-the-release` / the findings become rules | `ede1da7` | R-010 applied to its own release and STILL missed: the local suite was green because a submodule checkout has `.git` as a FILE, and `os.path.isdir` switched the release-gap check off silently — in the only checkout this family is developed in. Fixed to `exists`, and the precondition now discloses when it cannot look. The class was already recorded twice in this repository and this instance was missed both times. **`v1.75.0` shipped** | yes |
