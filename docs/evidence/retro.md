...
- **2026-09-01 — the second axis of blindness, and a version taken from under another run** (base `8e92ce8`, v1.81.1 → v1.82.0). B-114: `certification.md` decoupled the three readers from each other and never decoupled the fixer from the grader — tier blindness was horizontal only, and the vertical half is the one an optimisation removes first, because the fixer already holds the context. Stated as a dispatch rule rather than a check, since `certify` cannot detect the collapse from a report: a self-grading optimiser's reports stay sincere. The two-severity rule gained the scope it shipped without — two severities for a **finding**, three verdicts for a **stochastic check**, with the third pointed at `agent-evals` rather than added to `certify` (binary pass/fail measures 0% detection against 86% for trace fingerprinting). A fourth invisible class joined the three: internal state right, message about it stale, 21% → 4%. **Divergence recorded, four**: (1) **this run tagged `v1.81.0` onto a concurrent session's commit** — it chose the version by reading `package.json` at branch time without asking who held it, then pushed the tag immediately after a `gh pr merge` that had printed a conflict and NOT merged. The other session's release started before its stamp existed and failed on this very section's probe; nothing published, and the tag ruleset forbids deletion. Their account of it is better than the one this run wrote and theirs is what the retro keeps. Standing instruction #8 — a wrapper's exit status is not the verdict — fired for the fourth time in one programme, and this is the first time it cost somebody else a round. (2) **A commit shipped the index rather than the tree**: `git add -A`, then more edits, then a commit that captured the earlier snapshot — caught by reading what was actually in the push rather than trusting a green local gate. (3) **A regex substitution corrupted `marketplace.json` into unparseable JSON**; the retry parses the result before writing it. (4) **The lease expired three times mid-run** — TTL 2700s against a CI job of 450 steps and roughly an hour — and `acquire` reports *won* on an expired lock without refreshing its timestamp, so the lease reads as held and expired at once. Cleared by hand each time, with the ownership evidence the tool could not compute: `.agent-sync` lives inside a worktree created one minute before the lock. Both belong on `agent-sync`'s board. Ledger edits under lease `B-TP-verdicts`; nothing retired — no cold trigger hit.
