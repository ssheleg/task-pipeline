...
**What is still owed.** The other session's v1.55.0 and this one collide by version
number; neither is merged, and the sequencing is the operator's call rather than a thing
either agent should decide alone. `R-005` has not fired: this change widens a check and
its independent reader is due before merge, not after.
