...
| B-025 | **Stage 7's review loop has no declared cap.** `build.md:334` caps the stage-5 fix loop at five rounds per task and `loop-guard.md:60` caps stage re-entries at two; a review round is neither, and the run stamps record **10, 10, 8, 4, 3**. The guard never fired on any of them | `2026-08-10-pipeline-audit` / 0 | M | 2 | 3 | 0 | **6** | open | — |
| B-026 | **`.task-pipeline/run.md` has never been written.** `loop-guard.md:33` names it as the ledger for stage-level and program-level loops and calls detection mechanical; the directory holds only `build/`, so the detector has had no input in any run to date | `2026-08-10-pipeline-audit` / 0 | S | 2 | 3 | 0 | **6** | open | — |
| B-027 | **The pipeline routes user-facing work to `super-ux` and to nothing that owns how the product sounds or looks.** `copywriting` appears **0** times in the repository; `sheleg-design` appears **once**, as a name in a family list at `README.md:861` | `2026-08-10-pipeline-audit` / 0 | M | 2 | 3 | 0 | **6** | open | — |
| B-028 | **No stage prints which pipeline, which module or which iteration is running.** The run checklist exists at `stages.md:18` marked *"copy it, tick it"* — no gate requires it, nothing ties it to the module map or a board id, and no run of this session printed one | `2026-08-10-pipeline-audit` / 0 | M | 2 | 3 | 0 | **6** | open | — |
| B-029 | **Every lesson the pipeline learns dies in the repository that learned it.** Retro entries are written per project and read by nobody upstream; the skill's own issue tracker has never held one | `2026-08-10-pipeline-audit` / 0 | M | 2 | 3 | 0 | **6** | open | — |
| B-030 | **Eleven stages and four manual gates for any change**, including a one-paragraph edit. The boundary exists in prose in `CLAUDE.md`; no stage-0 step measures the change and offers the short path, so the exemption depends on the agent remembering it | `2026-08-10-pipeline-audit` / 0 | M | 1 | 3 | 0 | 3 | open | — |
| B-031 | **The companion matrix is stale about super-ux's own surface.** It names six (`ux-foundation`, `ux-flows`, `ux-scenarios`, `ux-audit`, `/ux`, `/ux-lint`); super-ux ships eight skills and fifteen commands, including the whole `brand`/`copy` half | `2026-08-10-pipeline-audit` / 0 | S | 1 | 2 | 0 | 2 | open | — |
| B-032 | **`exposure.md:40`'s worked example disagrees with what ships.** The doctrine teaches `31 releases since the last human confirmation`; `test/validate.py:3236` prints `10 releases carry one`. The example also hardcodes the live `99`, which will drift | `2026-08-10-pipeline-audit` / 0 | S | 1 | 2 | 0 | 2 | open | — |
