...
| B-005 | The negative self-tests use **fixed `/tmp/<name>-copy` paths**, so two concurrent runs of `npm run test:all` overwrite each other's scratch. **Reproduced live 2026-08-09**: three overlapping runs during N2 corrupted each other's copies and none produced a verdict — this row predicted the failure and was read as a task list rather than a warning | `08-06-evidence-docs` #1 · `2026-08-09-planning-system` / N2 stage 6 | S | 3 | 2 | 3 | 6 | open | — |
