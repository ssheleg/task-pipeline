...
|---|---|---|---|
| R1 | `artifacts.md` names both new files in **both** maps and in the single-home table | the map's own reachability guard + `npm test` | N1, N2 |
| R2 | `docs/superpowers/backlog.md` — seeded at stage 0 when absent, picked up when present | template exists; doctrine states both paths; guard | N1 |
| R3 | Any stage may append a row mid-run, same rule as the ledger: *deferred out loud or lost* | doctrine + guard on row shape | N1 |
| R4 | Each loop iteration reads the board at the top and re-prioritises at the bottom; *"next up"* cites the measurement | `continuity.md` states it; the claim is checkable against the file | N5 |
| R5 | Every carry-over row homed `backlog` resolves to a real board id — **both directions** | guard, both ways, with a planted defect each way | N1 |
| R6 | `docs/superpowers/verification.md` — one row per REQ: shipped-in, auto verdict, human-confirmed date or `never` | template + guard | N2 |
| R7 | Stage 8 writes the row; stage 10 refuses a REQ with no row | `stages.md` + guard | N2 |
| R8 | The exposure index is computed and printed beside the verdict as a **disclosure** — inputs visible, plus the list of what to check | printed by the gate; claim-registry class so it cannot be restated | N3 |
| R9 | `/task-pipeline checkup` runs **standalone**, with no task in flight | command doc + the mode's own doctrine | N4 |
| R10 | The index is never rendered as a probability or a percentage | doctrine says so; guard rejects a `%` on that line | N3 |
...
| # | Module | REQ | Ships as |
|---|---|---|---|
| **N1** | **the board** — `backlog.md`, its template, both artifact-map rows, and the carry-over resolution in both directions | R1·R2·R3·R5 | v1.31.0 |
| N2 | the verification ledger — `verification.md`, its template, stage 8 writes, stage 10 requires | R1·R6·R7 | v1.32.0 |
| N3 | the exposure index — computed, printed as a disclosure with its inputs and the check-list | R8·R10 | v1.33.0 |
| N4 | the checkup mode — `/task-pipeline checkup`, standalone | R9 | v1.34.0 |
| N5 | the loop wiring — the iteration reads and re-prioritises, and *"next up"* cites the board | R4 | v1.35.0 |
