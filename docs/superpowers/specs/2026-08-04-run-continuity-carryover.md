...
| 5 | 10 Acceptance | `continuity.md` said *cancel the loop* and not *verify the cancel*; a teardown call accepted an unscheduled id and reported success, leaving the job alive | found after acceptance, before the tag — fixed in place rather than shipped and logged | REQ-013 | **resolved** in `continuity.md` -> *Parked at a manual gate*; retro entry 3 amended |
...
| 6 | 10 Acceptance | **Correction to row 2's attribution.** The ledger is append-only, so row 2 stands and this row states the record. Row 2 credits the phrase *"до конца на автомате"* as the operator's authorization for the write. It is not one: an instruction to finish the run autonomously is a generic signal, and row 2's own text says a generic signal is not a specific authorization. Reading it as consent is exactly the rationalization REQ-009 exists to block — and it was written by the session that wanted the answer to be yes | what actually closed T8: the operator said **"сделал"** — *I did it* — having applied the rule themselves; presence then verified by `grep` over `~/.claude/CLAUDE.md` (the threshold rule, the no-guess-without-a-signal clause, `PreCompact`, compact-don't-stop, and "not a fourth summary document" all found) | REQ-009 | **closed** — applied by the operator, verified by grep. The verification stays outside every guard's reach: no check in this repository can see a file in the home directory, which is why the wrong justification survived to the tag |
| 7 | post-run | **Row 3 closed** — `graphify-out/` refreshed. 88 changed files re-extracted (13 code via AST, 75 docs via four parallel subagents), graph 573 → 652 nodes / 914 edges / 57 named communities, health check clean (no dangling, missing, self-loop or collapsed edges), 827k subagent tokens | the no-subagent constraint was lifted by the operator asking for both ratchets | — | **done** — stage 0 of the next run may trust the graph again |
| 8 | post-run | **Row 4 closed** — Obsidian wiki synced from `968fc19` to `b0bbd62` (45 commits, five releases). Seven pages created (four project concepts, three global lessons), the routing lesson merged into the existing `prompt-rule-is-not-a-gate` rather than given a 146th page, overview/index/log/manifest updated | — | — | **done**, with one defect: rebuilding `hot.md`'s activity list by splitting on `\n- ` dropped everything after the third bullet — Active Threads and Key Takeaways were lost. The vault is not under git, so Active Threads was restored from what had been read into context and **Key Takeaways was regenerated and labelled as regenerated, not restored**. Lesson: never rebuild a list by splitting a document that continues past it |
