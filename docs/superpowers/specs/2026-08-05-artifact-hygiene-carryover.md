...
| C-004 | 0 | The `spec-plan-quality` run is parked at its stage-0 gate on branch `spec-plan-quality`, commit `dd3155e` | Sequenced behind this run on the operator's call | Unparked at stage 10: branch `spec-plan-quality` still holds `dd3155e`, its brief committed and its gate ungiven. Its `## Self-review` section now has a ready line — `hygiene: 6 checks, N findings, 0 open` | **closed at 10** |
...
| C-007 | 10 | The ladder walk found a defect class the gate does **not** catch: a blank line silently splits a markdown table. It happened in this very ledger | Not deferred out of laziness — a seventh check is worth **measuring** before it is written, which is the lesson this run learned twice | Carried as **REQ-015**, printed in the acceptance table as backlog rather than buried | open as a **printed ratchet**, the gate's own first backlog row → `B-004`|
...
- Unresolved: 0
- Closed: 6
- Carried as a printed ratchet: 1 (C-007 / REQ-015)
