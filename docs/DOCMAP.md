...
| A number stated in a living document | recompute it, or delete it | `test/validate.py` — **the claim registry**: one row per claim class, each naming the pattern that recognises the claim, the command that computes the truth, and the incident that earned the row. Reads digits **and** word forms; a quoted number is a citation and exempt; every class prints `agrees xN`/`dormant` beside the verdict. **Its corpus is every surface that states a number about now** — the named documents, `references/**`, `templates/**`, the shipped `scripts/**`, the Cursor rule, the command, and the registers this table names, where a number inside a **dated item** is a record and exempt (`DEC-0005`; on the board the discriminator is the `State` cell). Frozen accounts stay out: `CHANGELOG.md`, `docs/evidence/specs/`, `plans/` and `retro/`. A class matching one word order is a class that fires on nothing, which reads exactly like a class that passed. A count of an enumeration inside one sentence is not computable from outside it — those are **deleted**, not gated |
